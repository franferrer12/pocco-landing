# Fix Pocco Header - Solo cambiar contenido, NO recursos CSS/JS/imágenes
$html = Get-Content 'C:\Users\Equipo\Downloads\HOME FITZ.html' -Raw -Encoding UTF8

# Replace text content (NOT in src/href attributes for CSS/JS)
$html = $html -replace 'Fitz Club Madrid','Pocco Club'
$html = $html -replace 'FITZ CLUB','POCCO CLUB'

# Replace ONLY the header logo (keep other resources from fitzclubmadrid.com)
...
# Add class to Mesas VIP menu item
$html = $html -replace '(<a href="https://wa\.me/34614868148[^"]*" class="elementor-item)(")', '$1 elementor-item-vip$2'

# Save
$html | Out-File -FilePath 'D:\pocco-html-static\pocco.html' -Encoding UTF8

Write-Host "✅ HTML corregido - recursos cargan desde Fitz, contenido de Pocco" -ForegroundColor Green
